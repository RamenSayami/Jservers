	...
	public void run() {
		...
		while (true) {
			try {
				...
				Order row = (Order) obj;
				DefaultTableModel model = (DefaultTableModel) ui.Cash_table
						.getModel();
				model.addRow(new Object[] { row.getTableNo(),
						row.getDishName(), row.getQuantity(),
						row.getStatusString(), row.getUnitPrice() });
